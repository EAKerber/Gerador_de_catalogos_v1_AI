/* Integração dos contratos incrementais no AuthoringKit autocontido. */
const fs = require("fs");
const os = require("os");
...
for (const fileName of contracts) {
  ...
  assert(buildSource.includes(`"${fileName}"`), `O build canônico não sincroniza ${fileName}.`);
  assert(compilerSource.includes(`"${fileName}"`), `O compilador não carrega ${fileName}.`);
  assert(indexSource.includes(`src="app/${fileName}"`), `O editor não carrega ${fileName} estaticamente.`);
}
