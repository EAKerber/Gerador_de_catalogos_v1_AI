/* DB-05.18.17 — fact no manifesto, runtime e build canônico. */
const fs = require("fs");
...
assert(indexSource.includes('src="app/fact-recipe-contract.js"'), "A página não carrega o contrato fact estaticamente.");
...
assert(buildSource.includes('"fact-recipe-contract.js"'), "O build canônico não copia o contrato fact.");
assert(buildSource.includes("CatalogFactRecipeContract.install()") && buildSource.includes("recipes: capabilities.recipes"), "O build não projeta fact entre as receitas integradas.");
