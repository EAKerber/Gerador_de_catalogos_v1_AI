...
(async () => {
  ...
  await page.locator("[data-product-card-organization]").selectOption("hero-grid-strip");
  ...
  const structure = await page.evaluate(() => {
    ...
    return { compositionId: composition?.id, hero: hero?.children.length, grid: grid?.children.length, strip: strip?.children.length, contextId: CatalogEditor.store.getState().editor.editingContextId, gridId: grid?.id, history: CatalogEditor.store.getHistoryState() };
  });
  ...
  await page.locator("[data-batch-table-schema]").selectOption("measured");
  await page.locator("[data-batch-table-schema-apply]").click();
