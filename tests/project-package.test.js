...
(async () => {
  ...
  assert(capabilities.editor.increment === "05.17" && capabilities.recipes.length === 5 && capabilities.capabilities.officialSectionRecipes && capabilities.capabilities.batchGeometry, "O kit não publicou a geometria de grupo do 05.17.");
  ...
  console.log("✓ Pacote, hashes, assets, CatalogSource, gates e AuthoringKit 1.5.7 validados.");
})().catch(error => {
  console.error(error);
  process.exitCode = 1;
});
