...
(async () => {
  ...
  assert(capabilities.editor.increment === "05.17" && capabilities.recipes.length === 5 && capabilities.capabilities.officialSectionRecipes && capabilities.capabilities.batchGeometry && capabilities.capabilities.bulkCollectionEditing, "O kit não publicou a geometria e as coleções em lote do 05.17.");
  ...
  console.log("✓ Pacote, hashes, assets, CatalogSource, gates e AuthoringKit 1.5.8 validados.");
})().catch(error => {
  console.error(error);
  process.exitCode = 1;
});
