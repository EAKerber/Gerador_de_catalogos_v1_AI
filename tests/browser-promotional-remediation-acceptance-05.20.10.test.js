...
(async () => {
  ...
  const fixture = await page.evaluate(contract => {
    ...
    const amountValues = ["3,99", "4,99", "5,99", "6,99"];
    const byRole = (root, role) => {
      let found = null;
      const visit = component => {
        if (found) return;
        if (component.props?.recipeRole === role) found = component;
    ...
    store.runCompoundChange({ type: "promotional-price-fixture-updated" }, () => {
      ids.forEach((id, index) => {
        const offer = store.findComponent(id).component;
        store.updateComponent(byRole(offer, "amount").id, { props: { content: amountValues[index] } });
      });
    });
    return {
      stripId: strip.id,
      ids,
      amountValues,
      historySteps: store.getHistoryState().undoCount,
      document: JSON.parse(JSON.stringify(store.getState()))
    };
  }, contract);
    ...
    return page.evaluate(({ ids, media }) => {
      ...
      const offers = ids.map(id => {
        ...
        return {
          ...
          amountText: amountText?.textContent?.trim() || "",
          amountClientWidth: amountText?.clientWidth || 0,
          amountScrollWidth: amountText?.scrollWidth || 0,
          currencyText: currencyText?.textContent?.trim() || "",
          currencyClientWidth: currencyText?.clientWidth || 0,
          currencyScrollWidth: currencyText?.scrollWidth || 0,
  ...
  function validate(snapshot) {
    ...
    snapshot.offers.forEach((offer, index) => {
      ...
      assert(offer.amountText === fixture.amountValues[index], `Oferta ${index + 1}: valor promocional incompleto (${offer.amountText || "vazio"}).`);
      assert(offer.amountScrollWidth <= offer.amountClientWidth + 1, `Oferta ${index + 1}: valor promocional truncado visualmente.`);
      assert(offer.currencyText === "R$", `Oferta ${index + 1}: moeda incompleta (${offer.currencyText || "vazia"}).`);
      assert(offer.currencyScrollWidth <= offer.currencyClientWidth + 1, `Oferta ${index + 1}: moeda truncada visualmente.`);
  ...
  const historySteps = fixture.historySteps;
