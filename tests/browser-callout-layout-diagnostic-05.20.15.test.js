...
(async () => {
  ...
  const fixture = await page.evaluate(() => {
    ...
    const createScenario = (frame) => {
      ...
      store.updateComponent(callout.id, { frame });
    ...
    return {
      wide: createScenario({ x: 24, y: 24, width: 430, height: 150 }),
      compact: createScenario({ x: 500, y: 24, width: 220, height: 210 })
    };
  });

  await page.waitForFunction(items => items.every(item => document.querySelector(`[data-component-id="${item.rootId}"]`)), Object.values(fixture));
  const browserMetrics = await page.evaluate(fixture => {
    ...
    const readScenario = source => {
      const readRole = role => {
        const id = source.roles[role];
        ...
        return {
          ...
          lineHeight,
          estimatedLines: lineHeight ? Math.round((p?.scrollHeight || 0) / lineHeight) : 0,
      ...
      const root = store.findComponent(source.rootId).component;
      const content = store.findComponent(source.roles.content)?.component;
      const icon = store.findComponent(source.roles.icon)?.component;
      return {
        ...
        icon: { modelFrame: icon ? { ...icon.frame } : null, domFrame: rect(document.querySelector(`[data-component-id="${source.roles.icon}"]`)) },
        content: { modelFrame: content ? { ...content.frame } : null, domFrame: rect(document.querySelector(`[data-component-id="${source.roles.content}"]`)), layout: content?.layout || null },
    ...
    return {
      ...
      wide: readScenario(fixture.wide),
      compact: readScenario(fixture.compact)
    };
  }, fixture);
  ...
  await browser.close();
  browser = null;
  console.log("✓ DB-05.20.15 registrou a geometria interna real dos callouts amplo e compacto.");
})().catch(async error => {
