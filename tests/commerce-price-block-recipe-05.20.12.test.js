...
for (const role of ["price-block", "currency", "amount"]) assert(roles.has(role), `Papel obrigatório ausente: ${role}.`);
...
assert(buildSource.includes('"commerce-price-block-recipe-contract.js"'), "Build Developer B não sincroniza o contrato.");
assert(buildSource.includes('"commerce-price-block"'), "Manifesto Developer B não inclui a receita.");
assert(mainSource.includes('file: "commerce-price-block-recipe-contract.js"'), "Editor não carrega o contrato.");
