  ...
  function install() {
    ...
    class TableBindingOverridesStore extends BaseStore {
      replaceTableRowsBulk(componentId, entries = [], options = {}) {
        ...
        const synchronized = options.bindingSync === true || Number(this.bindingSyncDepth) > 0;
        const fields = card?.binding?.productId && !synchronized ? overriddenFields(entries, options) : [];
        ...
        try {
          ...
          const restoredTable = this.findComponent(componentId)?.component;
          const restoredCard = restoredTable?.type === "data-table" ? this.getProductCard?.(restoredTable.id) : null;
          if (restoredCard?.binding) restoredCard.binding.overrides = previous;
          throw error;
        }
      }
    }
