...
| Parcial 05.21 | Camadas como instrumento de seleção | Subárvores recolhíveis, revelação/rolagem, contexto, seleção, ancestrais e descendentes possuem estados distintos. | Busca e intervalo continuam adiados. A reordenação entre irmãos foi promovida a oportunidade P2 de baixo risco: deve reutilizar `reorderComponent`, respeitar grupo/slot, reflow e undo, e oferecer controle acessível por teclado além de eventual drag. Reparenting, cruzamento entre slots e alteração do modelo de empilhamento permanecem em discovery. |
