...
| 8 | P2 · concluído 05.54 | Fundo útil de imagens factuais | O fluxo de assets exige `contain` e, quando necessário, expansão não generativa do canvas com pixels neutros, novo tamanho/hash e proveniência; o produto factual não pode ser completado ou reinterpretado. | Cumprido: o padrão `factual-image-neutral-canvas` descreve encaixe, transformação permitida, integridade e aprovação do asset. |
