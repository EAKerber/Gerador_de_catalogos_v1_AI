(function () {
  ...
  function improvedCallout(current) {
  ...
  function patchRecipes() {
    ...
    if (current.__benefitCalloutLegibilityVersion === CONTRACT_VERSION) return true;
    const source = current.get("section-tip-callout");
    if (!source) return false;
    const callout = improvedCallout(source);
    ...
    Object.defineProperty(api, "__benefitCalloutLegibilityVersion", { value: CONTRACT_VERSION });
  ...
  function install() {
    const registryInstalled = patchRegistry();
    const recipeInstalled = patchRecipes();
    const stylesInstalled = typeof document === "undefined" ? true : installStyles();
    return registryInstalled && recipeInstalled && stylesInstalled;
  }

  window.CatalogBenefitCalloutLegibilityContract = Object.freeze({
