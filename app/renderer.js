  ...
  class EditorRenderer {
    ...
    renderLayers(state) {
      ...
      const renderNodes = (children, depth = 0, selectedAncestorId = null) => children.map((component, index) => {
        ...
        return `
          <div class="layer-node" data-layer-depth="${depth}">
            <div class="layer-row" style="--layer-depth:${depth}">
              <button type="button" class="layer-item" data-layer-id="${escapeHtml(component.id)}" data-selected="${String(isSelected)}" data-within-selection="${String(Boolean(selectedAncestorId))}" data-active-context="${String(state.editor.editingContextId === component.id)}">
            ...
            ${hasChildren ? `<div class="layer-children">${renderNodes(component.children, depth + 1, selectedOwnerId)}</div>` : ""}
          </div>`;
      }).join("");
